...
export default function ListRender() {
    ...
    const counts2 = new Array(3).fill('대충채움'); // key 못씀
    ...
    return (
        <div>
            ...
            {counts2.map(item => <h6 key={item}>{item}</h6>)}
            {fiEmployees.map((item, index) => <h4 key={index}>{ `이름 : ${item.name} | 부서 : ${item.department}`}</h4>)}
        </div>
    )
}
